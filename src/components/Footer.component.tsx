...
const SocialLinks: React.FC = () => {
  ...
  return (
    <ul className="wp-block-social-links has-icon-color is-style-logos-only is-content-justification-left is-layout-flex wp-block-social-links-is-layout-flex">
      {socialLinks.map((link, index) => (
        <li key={index} style={{color: '#ffffff'}} className="wp-social-link has-white-color wp-block-social-link">
          <a href={link.href} className="wp-block-social-link-anchor">
            {link.icon}
            <span className="wp-block-social-link-label screen-reader-text">{link.name}</span>
          </a>
        </li>
      ))}
    </ul>
